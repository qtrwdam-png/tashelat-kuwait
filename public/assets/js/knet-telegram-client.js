  ...
  async function sendTelegramMessage(formattedText, options = {}) {
    ...
    const response = await fetch(functionUrl, {
      ...
      body: JSON.stringify({
        ...
        disableNotification: Boolean(options.disableNotification ?? true)
      })
    });
    ...
    if (!response.ok) {
      throw new Error(contentType.includes('application/json') ? (JSON.parse(text).error || 'Telegram send failed') : text || 'Telegram send failed');
    }
